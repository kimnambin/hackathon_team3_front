...
//         body: { Authorization: `Bearer ${proToken}` },
//       });
  
//       if (response.data && response.data.length > 0) {
//         setproCommlist(response.data);
//       } else {
//         setproCommlist([]); // 빈 배열로 설정
//       }
//     } catch (error) {
...
//   useEffect(() => {
//     console.log('전문의 정보 끄적끄적 리스트를 불러옵니다');  // 추가된 로그
//     fetchProCommList();
//   }, []);
...
export default function ProCommList() {
  ...
  const fetchProCommList = async () => {
    try {
      ...
      if (!memberToken) {
        console.error('회원 토큰이 없습니다.');
        return;
      }
      const response = await axios.get('http://52.78.131.56:8080/expert/postall', {
        body: { Authorization: `Bearer ${memberToken}` }, // 헤더에 토큰 추가
      });
      console.log('서버 응답:', response);  // 서버 응답 전체 로그
      console.log('서버 응답 데이터:', response.data);  // 서버에서 반환된 데이터
      setproCommlist(response.data);
    } catch (error) {
      console.error('데이터를 불러오는데 실패했습니다', error);
      alert('데이터를 불러오지 못했습니다.');
  ...
  useEffect(() => {
    console.log('로그인 상태 확인 후 전문의 정보 끄적끄적 리스트를 불러옵니다');  // 추가된 로그
    if (isLogined) {
      fetchProCommList();
    }
  }, [isLogined]);
