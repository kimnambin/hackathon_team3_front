...
const showcheckId = async (userId) => {
  console.log('클릭');
  try {
    // 서버에서 사용자 데이터 가져오기
    ...
    // 서버 응답 메시지로 중복 여부 확인
    const isDuplicate = message.includes('이미 존재하는 아이디 입니다.');
    ...
    const showcheckMsg = async () => {
      ...
      try {
        // 서버에서 사용자 데이터 가져오기
        const response = await axios.get(`http://52.78.131.56:8080/signup/checkId/${users}`);
        const users = response.data; // users 변수를 response 다음에 선언
        ...
        console.error('중복 확인 요청 실패22:', error);
        setCheckMsg('none');
      }
    };
  ...
  const postProData = () => {
    ...
    fetch('http://52.78.131.56:8080/signup/expert', {
      method: 'POST',
      headers: {
